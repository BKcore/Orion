...
class Context
{
        ...
        try {
            ...
            if($uri == '')
            {
                ...
                if(!array_key_exists($mode, $modelist))
                    throw new Exception("Default mode isn't registered in MODE_LIST", E_USER_ERROR, self::CLASS_NAME);
	 ...
	 * @param $preventInjection Set this to TRUE to escape variable content to prevent injections
	 */
	public static function post($var, $exceptIfNotSet, $preventInjection)
	{
            if(!isset($_POST[$var]))
            {
                if($exceptIfNotSet)
                    throw new Exception('"'.Security::prevenInjection($var).'" is not set in post data.');
                else
                    return null;
            }
            else
            {
                return ($preventInjection ? Security::prevenInjection($_POST[$var]) : $_POST[$var]);
            }			
	}
